...
function GradeAttendanceModal({ isOpen, onClose, onSubmit, student, mode }) {
  ...
  useEffect(() => {
    if (student) {
      if (mode === 'grade') {
        ...
        setValue('1'); // Default to incrementing 1 day of attendance
      }
    }
  }, [student, mode, isOpen]);
  ...
  const handleFormSubmit = (e) => {
    ...
    if (mode === 'grade') {
      ...
      if (numValue <= 0) {
        alert('Attendance increment must be greater than 0.');
        return;
      }
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-content" onClick={(e) => e.stopPropagation()}>
        <div className="modal-header">
          <h2>{mode === 'grade' ? 'Update Grade Record' : 'Record Attendance'}</h2>
        ...
        <form onSubmit={handleFormSubmit}>
          {mode === 'grade' ? (
            ...
            <div>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '1rem', padding: '0.75rem', backgroundColor: 'var(--bg-tertiary)', borderRadius: '6px', fontSize: '0.9rem' }}>
                <span style={{ color: 'var(--text-secondary)' }}>Current Days Attended:</span>
                <span style={{ fontWeight: '600', color: 'var(--success)' }}>{student.attendance || 0} Days</span>
              </div>
              
              <div className="form-group">
                <label className="form-label">Additional Days to Add</label>
                <input 
                  type="number" 
                  min="1" 
                  className="form-input" 
          ...
          <div className="modal-footer">
            ...
            <button type="submit" className="btn btn-primary">
              {mode === 'grade' ? 'Update Grade' : 'Log Attendance'}
            </button>
          </div>
        </form>
      </div>
